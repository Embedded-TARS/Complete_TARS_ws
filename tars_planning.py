...
class EnhancedLanePlanner:
    ...
    def detect_objects(self, frame):
        ...
        if boxes is not None:
            ...
            vehicle_roi_height = int(frame.shape[0] * 0.5)
    ...
    def calculate_lane_following(self, lane_center_x, image_center_x, detected_objects=None):
        ...
        if self.avoidance_active:
            ...
            print(f"🚧 [DEBUG] 회피 상태: {self.avoidance_direction}, 경과 프레임: {elapsed}")

            if elapsed < AVOIDANCE_PHASE_1_DURATION:
                # 첫 번째 단계: 장애물 반대 방향으로 크게 조향
                if self.avoidance_direction == 'left':
                    steering = -self.MAX_STEER * 0.7
                    # speed = 0.2
                    print(f"🚧 [DEBUG] 1단계 - 왼쪽 회피: steering={steering}")
                elif self.avoidance_direction == 'right':
                    steering = self.MAX_STEER * 0.8
                    print(f"🚧 [DEBUG] 1단계 - 오른쪽 회피: steering={steering}")
                speed = self.MIN_SPEED
                
            elif elapsed < AVOIDANCE_PHASE_1_DURATION + AVOIDANCE_PHASE_2_DURATION:
                # 두 번째 단계: 적당히 원래 방향으로 복귀
                if self.avoidance_direction == 'left':
                    steering = self.MAX_STEER * 0.6
                    print(f"🚧 [DEBUG] 2단계 - 왼쪽 복귀: steering={steering}")
                elif self.avoidance_direction == 'right':
                    steering = -self.MAX_STEER * 0.8
                    print(f"🚧 [DEBUG] 2단계 - 오른쪽 복귀: steering={steering}")
                speed = self.MIN_SPEED
                ...
                steering = 0.0
                speed = 0.42
                print(f"🚧 [DEBUG] 3단계 - 직진: steering={steering}, speed={speed}")
            elif elapsed < AVOIDANCE_PHASE_1_DURATION + AVOIDANCE_PHASE_2_DURATION + AVOIDANCE_PHASE_3_DURATION + AVOIDANCE_PHASE_4_DURATION:
                # 네 번째 단계: 장애물 방향으로 조향
                if self.avoidance_direction == 'left':
                    steering = self.MAX_STEER * 0.7
                    print(f"🚧 [DEBUG] 4단계 - 왼쪽 방향 조향: steering={steering}")
                elif self.avoidance_direction == 'right':
                    steering = -self.MAX_STEER * 0.8
                    print(f"🚧 [DEBUG] 4단계 - 오른쪽 방향 조향: steering={steering}")
                speed = self.MIN_SPEED
            else:
                # 회피 동작 종료 및 쿨다운 시작
                self.avoidance_active = False
                self.last_avoidance_frame = self.current_frame_count
                print("✅ [DEBUG] 회피 동작 완료 및 쿨다운 시작")
                return self.calculate_lane_following(lane_center_x, image_center_x, detected_objects)

            print(f"🚧 [DEBUG] 회피 동작 실행 중: {self.avoidance_direction} (frame {elapsed})")
            return speed, steering, 0.0

        # 차선 중심이 없으면 기본값 반환
        if lane_center_x is None:
            print("⚠️ [DEBUG] 차선 중심이 감지되지 않았습니다.")
            return 0.0, 0.0, 0.0
        ...
        if detected_objects:
            ...
            for obj in detected_objects:
                if obj['class'] in self.vehicle_classes:
                    ...
                    class_name = CLASS_INFO.get(obj['class'], {}).get('name', 'unknown')

                    print(f"🔍 [DEBUG] 차량 감지: {class_name}, 위치: {position}, 면적: {area}")

                    # 조건 1: 면적 충분
                    meets_area = area > self.min_detection_areas.get(obj['class'], 3000)

                    # 조건 2: 쿨다운 종료
                    cooldown_done = self.current_frame_count - self.last_avoidance_frame > self.AVOIDANCE_COOLDOWN_FRAMES

                    # 조건 3: 아직 회피 중 아님
                    not_avoiding = not self.avoidance_active

                    if position == 'right':
                        print("👉 [DEBUG] 차량이 오른쪽에 있음")
                        if meets_area and cooldown_done and not_avoiding:
                            self.avoidance_active = True
                            self.avoidance_start_frame = self.current_frame_count
                            self.avoidance_direction = 'left'
                            print("⚠️ [DEBUG] 회피 시작 → 왼쪽으로 이동")
                            return self.MIN_SPEED, 0.0, 0.0

                    elif position == 'left':
                        print("👈 [DEBUG] 차량이 왼쪽에 있음")
                        if meets_area and cooldown_done and not_avoiding:
                            self.avoidance_active = True
                            self.avoidance_start_frame = self.current_frame_count
                            self.avoidance_direction = 'right'
                            print("⚠️ [DEBUG] 회피 시작 → 오른쪽으로 이동")
                            return self.MIN_SPEED, 0.0, 0.0

                    else:
                        print(f"❓ [DEBUG] 차량 위치 정보가 명확하지 않음: {position}")
        ...
        print(f"🚗 [DEBUG] 정상 주행: 속도={speed}, 조향={steering}, 편차={deviation}")
